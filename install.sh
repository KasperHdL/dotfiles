...
sudo apt-get --assume-yes install nodejs npm cmake bzr rpm wget git curl blender build-essential irssi screen chromium-browser git gvfs-bin pm-utils make automake powertop python thermald ttf-ancient-fonts unzip zsh qemu libunwind8 gettext libssl-dev libcurl3-dev zlib1g libicu-dev
...
sudo apt-get update && sudo apt-get --assume-yes install mono-complete mono-devel ca-certificates-mono
